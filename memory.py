...
Based on Fringe Memory Game by Lulets
...
side_logo = 'images/Emoji_logo.png'
new_game_btn = 'images/New_game.png'
quit_game_btn = 'images/quit_button.png'
img_1 = 'images/Heart_emoj.jpg'
img_2 = 'images/Angry_emoj.png'
img_3 = 'images/Sleep_emoj.png'
img_4 = 'images/Shocked_emoj.jpg'
img_5 = 'images/Smirk_emoj.jpg'
img_6 = 'images/Happy_emoj.jpg'
img_7 = 'images/Sad_emoj.png'
img_8 = 'images/Party_emoj.png'
img_lst = [img_1, img_2, img_3, img_4, img_5, img_6, img_7, img_8]

# Texts to be displayed on screen
left_txt1 = 'Memory game based on the'
left_txt2 = 'iphone emojis.'
left_txt3 = 'Flip the emojis hidden'
left_txt4 = 'pics and match two'
...
# Font to be used
pixel_font = 'fonts/pixel_art/Monstora.otf'
...
def main():
    ...
    while mainloop:
        ...
        text = " Emoji Memory Game FPS: {0:.2f}   Playtime: {1:.2f}".format(fps_clock.get_fps(), playtime)
        pygame.display.set_caption(text)

        tile_pos = get_tile_at_pos(x_mouse_pos, y_mouse_pos)
        if (tile_pos[0] is None and tile_pos[1] is None) and mouse_clicked:
            new_game_rect = pygame.Rect(30, WINDOW_HEIGHT - 300, LEFT_PANEL - 50, 65)
        ...
        #Quits the game
        def save(tiles):
            with open('tiles.json', 'w') as output_file:
                new =json.dumps(flipped_tiles,default=lambda o: o.__dict__, sort_keys=True, indent= 4)
                json.dump(new,output_file)

        if (tile_pos[0] is None and tile_pos[1] is None) and mouse_clicked:
            quit_game_rect = pygame.Rect(30, WINDOW_HEIGHT - 200, LEFT_PANEL - 50, 65)
            if quit_game_rect.collidepoint(x_mouse_pos, y_mouse_pos):
                tiles = flipped_tiles
                save(tiles)
                pygame.quit()
                sys.exit()
...
def draw_control_panel(width, height):
    ...
    new_logo_x = 30
    new_logo_y = height - 300

    screen.blit(top_logo, (top_logo_x, top_logo_y))
    screen.blit(new_game_logo, (new_logo_x, new_logo_y))

    #Quit button at left side panel
    quit_game_logo = pygame.image.load(quit_game_btn).convert()
    quit_game_logo = pygame.transform.scale(quit_game_logo, (width - 50, 65))
    quit_logo_x = 30
    quit_logo_y = height - 200

    screen.blit(top_logo, (top_logo_x, top_logo_y))
    screen.blit(quit_game_logo, (quit_logo_x, quit_logo_y))
    ...
    for text in range(len(left_panel_texts)):
        ...
        text_rect.center = (0, 200 + text * 50)
        text_rect.left = 25
